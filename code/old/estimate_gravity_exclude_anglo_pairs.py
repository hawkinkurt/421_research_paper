...
"""
Estimates gravity models EXCLUDING only Anglo-to-Anglo country pairs:
US-UK, US-CA, US-AU, US-NZ, UK-CA, UK-AU, UK-NZ, CA-AU, CA-NZ, AU-NZ

This KEEPS flows between English-speaking and non-English countries:
US-France, UK-Germany, Australia-Japan, etc.

Compare with estimate_gravity_non_anglo.py which excludes ALL flows
involving English-speaking countries.
"""
...
ANGLO_COUNTRIES = ['US', 'GB', 'CA', 'AU', 'NZ']
...
print("=" * 70)
print("GRAVITY MODEL: EXCLUDING ONLY ANGLO-TO-ANGLO PAIRS")
print("=" * 70)
print(f"\nAnglo countries: {', '.join(ANGLO_COUNTRIES)}")

print(f"\n=== FULL DATASET ===")
print(f"Total observations: {len(df_full)}")

# Count Anglo-to-Anglo pairs before filtering
anglo_to_anglo = df_full[
    (df_full['importer'].isin(ANGLO_COUNTRIES)) &
    (df_full['exporter'].isin(ANGLO_COUNTRIES))
    ]
print(f"Anglo-to-Anglo pairs: {len(anglo_to_anglo)}")

# Filter: KEEP pairs where at least one country is NOT Anglo
# i.e., EXCLUDE only pairs where BOTH are Anglo
df = df_full[
    ~(df_full['importer'].isin(ANGLO_COUNTRIES) &
      df_full['exporter'].isin(ANGLO_COUNTRIES))
].copy()
...
# Breakdown of remaining flows
anglo_to_nonanglo = df[
    (df['exporter'].isin(ANGLO_COUNTRIES)) &
    (~df['importer'].isin(ANGLO_COUNTRIES))
    ]
nonanglo_to_anglo = df[
    (~df['exporter'].isin(ANGLO_COUNTRIES)) &
    (df['importer'].isin(ANGLO_COUNTRIES))
    ]
nonanglo_to_nonanglo = df[
    (~df['exporter'].isin(ANGLO_COUNTRIES)) &
    (~df['importer'].isin(ANGLO_COUNTRIES))
    ]

print(f"\n=== FLOW COMPOSITION ===")
print(f"Anglo → Non-Anglo: {len(anglo_to_nonanglo)} ({len(anglo_to_nonanglo) / len(df) * 100:.1f}%)")
print(f"Non-Anglo → Anglo: {len(nonanglo_to_anglo)} ({len(nonanglo_to_anglo) / len(df) * 100:.1f}%)")
print(f"Non-Anglo → Non-Anglo: {len(nonanglo_to_nonanglo)} ({len(nonanglo_to_nonanglo) / len(df) * 100:.1f}%)")
...
print("\nCoefficients:")
for var in ['const', 'log_gdp_importer', 'log_gdp_exporter', 'log_dist']:
    coef = model1.params[var]
    se = model1.bse[var]
    pval = model1.pvalues[var]
    stars = "***" if pval < 0.01 else "**" if pval < 0.05 else "*" if pval < 0.1 else ""
    print(f"  {var:<20} {coef:>10.4f} ({se:.4f}) {stars}")

# =============================================================================
# MODEL 7: FULL MODEL + FILM INCENTIVE
# =============================================================================
...
# Run all 7 models for comparison
X2_vars = ['log_gdp_importer', 'log_gdp_exporter', 'log_dist', 'contig', 'comlang_off', 'col_dep_ever']
X2 = sm.add_constant(df_est[X2_vars])
model2 = sm.OLS(y, X2).fit(cov_type='HC1')

X3_vars = X2_vars + ['log_remoteness_imp', 'log_remoteness_exp']
...
X5_vars = X4_vars + ['efw_importer', 'efw_exporter']
X5 = sm.add_constant(df_est[X5_vars])
model5 = sm.OLS(y, X5).fit(cov_type='HC1')

X6 = sm.add_constant(pd.concat([df_est[X5_vars], year_dummies], axis=1))
...
print("\n" + "=" * 70)
print("COEFFICIENT COMPARISON ACROSS MODELS (Excluding Anglo-Anglo Pairs)")
print("=" * 70)
...
# =============================================================================
# ROBUSTNESS CHECK: COUNTRY FIXED EFFECTS
# =============================================================================

print("\n\n" + "=" * 70)
print("ROBUSTNESS CHECK: IMPORTER/EXPORTER FIXED EFFECTS")
print("=" * 70)
...
# Use filtered df for FE analysis
df_for_fe = df.dropna(subset=['log_trade_real'] + pair_vars + ['log_gdp_importer', 'log_gdp_exporter',
                                                               'remoteness_importer', 'remoteness_exporter']).copy()

df_for_fe['log_remoteness_imp'] = np.log(df_for_fe['remoteness_importer'])
df_for_fe['log_remoteness_exp'] = np.log(df_for_fe['remoteness_exporter'])
...
# --- Comparison ---
print("\n" + "=" * 70)
print("COMPARISON: STANDARD vs FIXED EFFECTS")
print("=" * 70)
...
print("\n" + "=" * 70)
print("KEY FINDINGS: INCENTIVE EFFECT")
print("=" * 70)

print(f"""
EXCLUDING ONLY ANGLO-TO-ANGLO PAIRS:
...
print("\n" + "=" * 70)
print("SUMMARY: THREE SAMPLE COMPARISONS")
print("=" * 70)

print(f"""
                              Full Sample    Excl. Anglo-Anglo    Excl. All Anglo
                              -----------    -----------------    ---------------
Observations:                      1,504             {len(df_est):,}                590
Anglo-to-Anglo pairs:                Yes                   No                 No
Anglo-to-NonAnglo pairs:             Yes                  Yes                 No
NonAnglo-to-Anglo pairs:             Yes                  Yes                 No
NonAnglo-to-NonAnglo pairs:          Yes                  Yes                Yes

This sample keeps US→France, UK→Germany, etc. but removes US→UK, AU→NZ, etc.

Key question: Is the incentive effect driven by:
  (a) Anglo-to-Anglo flows (US studio → UK/AU/NZ production)? 
  (b) Anglo-to-NonAnglo flows (US studio → France/Germany production)?
  (c) General film trade patterns?

Compare incentive coefficients across the three samples to understand
which flows drive the effect.
""")
...
comparison_df.to_csv(f"{output_dir}/model_comparison_excl_anglo_pairs.csv", index=False)

print(f"\nResults saved to: {output_dir}/model_comparison_excl_anglo_pairs.csv")